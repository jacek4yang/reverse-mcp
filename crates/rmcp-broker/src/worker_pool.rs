...
impl WorkerPool {
    ...
    pub fn ensure_worker_exe(&mut self) -> Result<PathBuf> {
        ...
        for c in candidates {
            ...
            // An idalib-feature exe links ida.dll at load time, so probe
            // with the IDA dir (same resolution as spawn) on PATH or the
            // loader kills the process with STATUS_DLL_NOT_FOUND.
            let probed = self
                .probe_command(&c)
