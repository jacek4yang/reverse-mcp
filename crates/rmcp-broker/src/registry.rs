...
fn defs() -> Vec<ToolDef> {
    vec![
        ...
        ToolDef {
            ...
            description: "Deobfuscation (#9 analysis + #46 transforms). task=analyze (default): analysis-only pass engine over one function (flattening, opaque branches, indirect transfers, junk no-ops, tail jumps) with evidence and proposals; no IDB change. task=propose: build a bounded transform plan (T1_opaque_branch / T2_junk_removal / T3_indirect_materialize patch sites with evidence; T4_unflatten reports requires_microcode) - still analysis-only. task=validate: re-check a plan against the live DB (outside-function sites, inbound xrefs, unreadable bytes) and return ok|rejected+reason. task=apply: snapshot first, then apply the plan through the audited mutation machinery with an expected_revision guard; returns before/after decompile fingerprints + rollback pointer. task=rollback: restore the last snapshot (audited). Every mutation is explicit and per-function; rejected plans never touch the IDB.",
            schema: json!({"type": "object", "properties": {"db": {"type": "string"}, "task": {"type": "string", "enum": ["analyze", "propose", "validate", "apply", "rollback"]}, "target": {"type": "string"}, "kind": {"type": "string", "enum": ["T1_opaque_branch", "T2_junk_removal", "T3_indirect_materialize", "T4_unflatten"]}, "plan": {"type": "object", "description": "plan object returned by task=propose"}, "max_passes": {"type": "integer", "maximum": 16}, "expected_revision": {"type": "integer"}}, "required": ["target"]}),
        },
        ToolDef {
            name: "ida_sig",
