...
uint32_t idalib_qflow_graph_calc_block_type(const qflow_chart_t *cfg, size_t n) {
  return static_cast<uint32_t>(cfg->calc_block_type(n));
}

size_t idalib_qflow_graph_size(const qflow_chart_t *cfg) {
...
// DECLARE_COMPARISONS(func_t) declares func_t::compare but the kernel does not
// export it (the stub ida.lib has no symbol for it). Provide an out-of-line
// definition with identical semantics (compare by start_ea, then end_ea).
