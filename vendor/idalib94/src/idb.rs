...
impl IDB {
    ...
    pub fn current() -> Result<Self, IDAError> {
        ...
        Ok(Self {
            decompiler: unsafe { idalib_hexrays_init(autocxx::c_int(0)) },
            _guard,
            _marker: PhantomData,
        })
    }
    ...
    pub fn format_cfunc_decls_with<'a>(
        ...
        unsafe { idalib_format_cfunc_decls(cfunc.inner_ptr(), options.bits()) }
...
impl<'a> Iterator for EntryPointIter<'a> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        while self.index < self.limit {
            ...
            let name =
                unsafe { crate::ffi::ffix::idalib_entry_name(autocxx::c_ulonglong(ordinal.0)) }
                    .unwrap_or_default();
            let ordinal = usize::try_from(ordinal.0).unwrap_or(0);
            ...
            return Some((ordinal, addr.into(), name));
        }
