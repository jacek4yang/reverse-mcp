...
impl<'a> BasicBlock<'a> {
    ...
    pub fn len(&self) -> usize {
        unsafe { (*self.as_range_t()).size() as _ }
    }
...
impl<'a> FunctionCFG<'a> {
    fn calc_block_type_via_helper(&self, id: usize) -> fc_block_type_t {
        let v = unsafe {
            idalib94_sys::ffix::idalib_qflow_graph_calc_block_type(
    ...
    pub fn block_by_id(&self, id: BasicBlockId) -> Option<BasicBlock<'_>> {
        ...
        let kind = self.calc_block_type_via_helper(id);
    ...
    pub fn exit(&self) -> Option<BasicBlock<'_>> {
        let qty = self.blocks_count();
        if qty == 0 {
            return None;
        }
        self.block_by_id(qty - 1)
    }

    pub fn blocks_count(&self) -> usize {
        unsafe {
            idalib94_sys::ffix::idalib_qflow_graph_size(
                self.flow_chart.as_ref().expect("valid pointer"),
            )
        }
    }
