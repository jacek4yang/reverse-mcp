...
include_cpp! {
    ...
    extern_cpp_type!("range_t", reverse_ida_sys::range_t)
    generate_pod!("uval_t")
    ...
    // idp
    // get_proc_index() is an idp.hpp inline whose QASSERT expands to a read
    // of the `under_debugger` DATA export of ida.dll; data imports cannot be
    // delay-loaded (LNK1194) and reverse-mcp never calls it. Block the
    // wrapper so no code referencing it is emitted (kept out by the
    // -Dunder_debugger rename in build.rs as a belt-and-braces measure).
    block!("processor_t::get_proc_index")
    ...
    extern_cpp_type!("gdl_graph_t", reverse_ida_sys::gdl_graph_t)
    generate!("qflow_chart_t")
    // patched: gdl_graph_t is abstract; autocxx cannot construct it (MSVC)
    ...
    */
    ...
    generate!("carg_t")
    // carglist_t::print overloads are declared in hexrays.hpp but defined
    // outside the import libraries we link; nothing in reverse-mcp calls
    // them, so keep the generated wrappers out of the binary.
    block!("carglist_t::print")
    generate!("carglist_t")

    extern_cpp_type!("cblock_t", reverse_ida_sys::cblock_t)
    extern_cpp_type!("cfunc_t", reverse_ida_sys::cfunc_t)
    extern_cpp_type!("citem_t", reverse_ida_sys::citem_t)
    extern_cpp_type!("cinsn_t", reverse_ida_sys::cinsn_t)
    extern_cpp_type!("cexpr_t", reverse_ida_sys::cexpr_t)
    extern_cpp_type!("cswitch_t", reverse_ida_sys::cswitch_t)
    extern_cpp_type!("ctry_t", reverse_ida_sys::ctry_t)
    extern_cpp_type!("cthrow_t", reverse_ida_sys::cthrow_t)
    ...
    // xref
    extern_cpp_type!("xrefblk_t", reverse_ida_sys::xrefblk_t)
...
pub mod hexrays {
    ...
    pub use super::ffi::{carg_t, carglist_t};
    // autocxx-idalib 0.30 generates the cfuncptr_t typedef as a plain alias
    // to qrefcnt_t<cfunc_t> which is not a UniquePtrTarget; the concrete
    // wrapper type is what owns the refcount across the FFI boundary.
    pub use super::ffi::qrefcnt_t_cfunc_t_AutocxxConcrete as cfuncptr_t;
    pub use super::ffix::{
        ...
        idalib_hexrays_cfuncptr_inner, idalib_hexrays_decompile_func, idalib_hexrays_init,
        idalib_hexrays_term,
    };
...
pub mod ffix {
    ...
    unsafe extern "C++" {
        ...
        type range_t = reverse_ida_sys::range_t;
        ...
        unsafe fn idalib_qflow_graph_calc_block_type(f: *const qflow_chart_t, n: usize) -> u32;
        unsafe fn idalib_qflow_graph_size(f: *const qflow_chart_t) -> usize;
        ...
        unsafe fn idalib_disasm_line(ea: c_ulonglong) -> String;
...
pub use reverse_ida_sys::range_t;

/// ea_t as autocxx's c_ulonglong wrapper (matches generated ffi).
#[allow(non_camel_case_types)]
pub type ea_t = autocxx::c_ulonglong;
...
pub mod insn {
    ...
    pub use reverse_ida_sys::insn_t;
...
pub mod func {
    pub use super::ffi::{
        add_func, calc_thunk_func_target, del_func, fc_block_type_t, func_t, get_func,
        get_func_num, get_func_qty, getn_func, lock_func,
    };
    ...
    pub use super::ffix::{
        ...
        idalib_qbasic_block_succs, idalib_qflow_graph_calc_block_type,
