...
void idalib_msg(const char *m) {
  // msg()/vmsg() are kernwin.hpp inlines that read the `callui` data
  // export; a static data import is incompatible with delay-loading
  // ida.dll (LNK1194). Dispatch ui_msg through the runtime resolver
  // instead - same pattern as idalib_open_database_quiet above.
  if (m && *m) {
    if (auto fp = idalib_callui_resolver()) {
      fp(ui_notification_t::ui_msg, "%s", m);
    }
  }
}
