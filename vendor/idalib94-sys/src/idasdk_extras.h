...
// Local stand-in for ida.dll's `under_debugger` data export: the build
// renames every reference (see build.rs, -Dunder_debugger=...). QASSERT
// failures still report through interr(); they just never breakpoint.
bool rmcp94_under_debugger = false;
...
// idalib_print_insn_mnem lives in ua_extras.h (upstream 0.10.1)
